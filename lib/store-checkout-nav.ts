import { STORE_THEME_COOKIE } from "@/lib/storefront-brand";
...
export function storeCheckoutBootScriptSource(brand: string): string {
  ...
  return `(()=>{try{var path=location.pathname||"";var html=document.documentElement;if(path.indexOf("/admin")===0||path.indexOf("/empezar")===0){try{sessionStorage.removeItem(${key});}catch(e){}html.classList.remove(${cls});var dead=document.getElementById(${id});if(dead)dead.remove();return;}var theme=null;try{var m=document.cookie.match(/(?:^|; )${STORE_THEME_COOKIE}=([^;]*)/);if(m)theme=JSON.parse(decodeURIComponent(m[1]));}catch(e){}if(theme&&theme.a){html.style.setProperty("--store-accent",theme.a);if(theme.h)html.style.setProperty("--store-accent-hover",theme.h);html.style.setProperty("--store-brand",theme.a);if(theme.h)html.style.setProperty("--store-brand-hover",theme.h);html.style.setProperty("--store-header-bg",theme.a);if(theme.w)html.style.setProperty("--store-wash",theme.w);}var k=${key};if(!sessionStorage.getItem(k))return;html.classList.add(${cls});var wash=(theme&&theme.w)||"#f7f7f7";var accent=(theme&&theme.a)||"#111111";var brandText=(theme&&theme.n)||${brandJson};if(!document.getElementById(${styleId})){var s=document.createElement("style");s.id=${styleId};s.textContent="html.${STORE_CHECKOUT_NAV_HTML_CLASS}::before{content:\\"\\";position:fixed;inset:0;z-index:2147483645;background:"+wash+";pointer-events:auto}html.${STORE_CHECKOUT_NAV_HTML_CLASS},html.${STORE_CHECKOUT_NAV_HTML_CLASS} body{overflow:hidden!important;background:"+wash+"!important}#${STORE_CHECKOUT_BOOT_OVERLAY_ID}{position:fixed;inset:0;z-index:2147483646;display:flex;align-items:center;justify-content:center;padding:1.5rem;background:"+wash+";pointer-events:auto;font-family:system-ui,-apple-system,sans-serif}#${STORE_CHECKOUT_BOOT_OVERLAY_ID} .store-checkout-boot-inner{position:relative;width:100%;max-width:24rem;text-align:center}#${STORE_CHECKOUT_BOOT_OVERLAY_ID} .store-checkout-boot-brand{margin:0;font-size:11px;font-weight:600;letter-spacing:.22em;text-transform:uppercase;color:"+accent+"}#${STORE_CHECKOUT_BOOT_OVERLAY_ID} .store-checkout-boot-spinner{width:2.75rem;height:2.75rem;margin:2rem auto 0;border-radius:9999px;border:3px solid color-mix(in srgb, "+accent+" 25%, transparent);border-top-color:"+accent+";animation:store-checkout-boot-spin .75s linear infinite}#${STORE_CHECKOUT_BOOT_OVERLAY_ID} .store-checkout-boot-msg{margin:2rem 0 0;min-height:3.25rem;font-size:1rem;font-weight:500;line-height:1.375;color:#292524}#${STORE_CHECKOUT_BOOT_OVERLAY_ID} .store-checkout-boot-note{margin:.75rem 0 0;font-size:12px;line-height:1.5;color:#78716c}@keyframes store-checkout-boot-spin{to{transform:rotate(360deg)}}";(document.head||html).appendChild(s);}function mount(){if(document.getElementById(${id}))return;var d=document.createElement("div");d.id=${id};d.setAttribute("role","status");d.setAttribute("aria-live","polite");d.setAttribute("aria-busy","true");d.innerHTML='<div class="store-checkout-boot-inner"><p class="store-checkout-boot-brand"></p><div class="store-checkout-boot-spinner" aria-hidden="true"></div><p class="store-checkout-boot-msg">Llevándote a finalizar tu compra…</p><p class="store-checkout-boot-note">Estamos armando tu checkout; un segundo y listo.</p></div>';var b=d.querySelector(".store-checkout-boot-brand");if(b)b.textContent=brandText;(document.body||html).appendChild(d);}if(document.body)mount();else document.addEventListener("DOMContentLoaded",mount);setTimeout(function(){try{sessionStorage.removeItem(k);}catch(e){}html.classList.remove(${cls});var o=document.getElementById(${id});if(o)o.remove();},15000);}catch(e){}})();`;
}
