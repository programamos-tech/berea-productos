-- Reportes: los abonos de crédito entran al mes/día en que se pagan.
-- Las RPCs vuelven a SECURITY DEFINER + filtro de tenant para no perder
-- order_payments por RLS de sucursal.

alter function public.admin_report_dashboard_agg(date, date, date, date, date, date) security definer;
alter function public.admin_report_period_line_metrics(date, date) security definer;
