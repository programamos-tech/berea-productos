-- Abonos de facturas POS a crédito. El saldo se calcula (total − sum abonos).
...
create index order_payments_order_paid_idx
  on public.order_payments (order_id, paid_at desc);
create index order_payments_tenant_paid_idx
  on public.order_payments (tenant_id, paid_at desc);
create index order_payments_branch_paid_idx
  on public.order_payments (branch_id, paid_at desc);

comment on table public.order_payments is
  'Abonos a facturas POS a crédito (POS:credit). No guardan saldo; pendiente = total − sum(amount_cents).';

alter table public.order_payments enable row level security;

create policy order_payments_select_admin
...
create policy order_payments_staff_tenant_isolation
...
create policy order_payments_staff_branch_isolation
on public.order_payments as restrictive for all to authenticated
using (
...
create trigger order_payments_set_tenant_id
before insert on public.order_payments
for each row execute function public.tg_set_tenant_id_from_staff();

create trigger order_payments_set_active_branch
before insert or update of branch_id, tenant_id on public.order_payments
...
-- Reportes: desglose efectivo/transferencia incluye abonos de crédito por paid_at.
