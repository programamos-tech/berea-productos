-- Anular abonos de crédito mal ingresados (motivo obligatorio).
-- Dejan de contar en saldo, caja y reportes; el registro se conserva.
...
comment on column public.order_payments.is_cancelled is
  'Abono anulado (error de carga). No suma a saldo ni a caja/reportes.';
comment on column public.order_payments.cancellation_reason is
  'Motivo de anulación del abono (auditoría).';

create index if not exists order_payments_active_paid_idx
  on public.order_payments (tenant_id, paid_at desc)
  where is_cancelled = false;

drop policy if exists order_payments_update_admin on public.order_payments;
create policy order_payments_update_admin
...
-- Reportes: abonos anulados no entran a efectivo/transferencia del periodo.
